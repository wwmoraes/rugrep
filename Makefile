BIN_NAME := rugrep
PARAMS := package Cargo.*
HT_VIEW_FILE = $(shell ls -t $(PROFILE_DIR)/heaptrack.rugrep.*.gz | head -n1)
HT_COMPARE_FILES = $(shell ls -t $(PROFILE_DIR)/heaptrack.rugrep.*.gz | head -n2)
